...
public class JavaSctiptMethods {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(action)) {
                if (action.equals("toast")) {
                    ...
                    btmDlg.show();
                }else if(action.equals("goToOrderDetail")){
                    goToOrderDetail(str);
                }else if(action.equals("changeClick")){
                    changeClick(str);
                }
            }
    ...
    private void changeClick(String json){
        try {
            JSONObject jsonObject = new JSONObject(json);
            System.out.println("changeClick方法被调用:" + jsonObject.toString());
            //解析json
            String phone = jsonObject.optString("phone");

            Toast.makeText(mActivity,phone,Toast.LENGTH_SHORT).show();


            String callback = jsonObject.optString("callback");//解析js回调方法

            JSONObject myjson = new JSONObject();
            myjson.put("phone_value", phone+"_"+ new Random().nextInt(10000));
            invokeJavaScript(callback, myjson.toString());
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    private void goToOrderDetail(String json){

        try {
            JSONObject jsonObject = new JSONObject(json);
            System.out.println("Demo goToOrderDetail方法被调用:" + jsonObject.toString());
            //解析json
